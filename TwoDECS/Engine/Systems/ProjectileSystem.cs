...
namespace TwoDECS.Engine.Systems
{
    public static class ProjectileSystem
    {
        ...
        {
            List<Guid> projectileEntities = playingState.Entities.Where(x => (x.ComponentFlags & ComponentMasks.Projectile) == ComponentMasks.Projectile).Select(x => x.ID).ToList();
            foreach (Guid projectileId in projectileEntities)
            {
                ...
                SpeedComponent speedComponent = playingState.SpeedComponents[projectileId];

                double vX = speedComponent.Speed * Math.Cos(directionComponent.Direction);
                double vY = speedComponent.Speed * Math.Sin(directionComponent.Direction);
